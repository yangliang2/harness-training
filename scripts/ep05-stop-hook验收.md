...
> 时长 5 分钟 | 交付物：Stop hook 验收配置 | 前置：第 1 期（谎报完成）、第 3 期（PreToolUse 机制）
...
【口播】
第一期你见过那个现场：agent 报告"全部测试通过"，实际测试根本没跑。如果你当时的解法是往 CLAUDE.md 里加一句"完成前必须跑测试"——那是第二期讲的东西：建议性约束，它遵守的概率很高，但违反的概率不为零。今天换个思路：让"完成"这个词，从它嘴里失效。

【画面】
前 5 秒回放第一期翻车片段（3 秒即可，打"第 1 期"角标）→ 切黑底字幕卡："完成的定义权，归谁？"
...
【画面】
录屏：hooks 参考文档 Stop 事件段落截图 → 字幕卡（静止 3 秒，观众自读，不占口播）：exit 2 = 拦截，stderr 失败信息回灌模型上下文 → 字幕卡："Stop hook = 出口处的检票口"
...
实录。仓库里已经埋了一个失败的测试，我给它的任务是写一个新函数，跟那个失败测试毫不相干。它写完，说"完成了"——看，Stop 被拦下，测试失败的两行输出塞了回去，它转去修。第二次想停，测试过了，放行。注意，它从头到尾都没觉得自己在撒谎，它只是用了它手里唯一的信号。
整个机制里，没有任何一步取决于它自不自觉。

【画面】
1. 录屏：`settings.json` 配置特写（语法高亮，逐行放大）→ `verify-done.sh` 脚本滚动展示，计数器段落打框；配置高亮时弹出字幕卡（观众自读，不占口播）：exit 0 = 放行 / exit 2 = 拦截，stderr 摘要回灌模型上下文（同第 3 期协议）
2. 实录：terminal 演示——agent 宣布完成 → Stop hook 拦截（stderr 红字失败摘要出现在会话中）→ agent 修复 → 第二次放行。等待片段加"已剪辑"字幕
3. 结尾字幕卡："确定性：测试不过 = 停不了"
...
【画面】
三张字幕卡依次弹出："只在失败时拦 / 快代理信号进热路径 / 最大轮次兜底"；讲第三点时在角落叠 Cursor hooks 文档 `loop_limit` 字段截图（字幕卡标注：内置参数，默认 5 次）
...
【画面】
1. 讲"自信地夸赞平庸之作"时叠 Anthropic《Harness design for long-running application development》博文截图（标注日期与作者）
2. 讲测试保护时画面一角复现第三期 PreToolUse 配置，路径从 `generated/` 改成 `tests/`，打"第 3 期"角标

### 4:25–4:45 信号保真度升级：Playwright MCP

【口播】
验证信号的保真度还能再升一档。左边是 agent 交上来的代码，它说"页面做完了"；右边是 Playwright MCP 实际渲染出来的样子。从"代码看着对"到"渲染结果对"——让评估者像用户一样去点你的应用，这是 Anthropic 那篇长任务 harness 文章里验收环节的标准做法。配置过程不占时间，成品在模板仓库。

【画面】
左右对比截图：代码 diff（看着合理） vs 渲染截图（布局明显崩坏）。不打攻击/失败过程，只放成品对比

### 4:45–5:10 半衰期判据 + 交付 + 作业

【口播】
半衰期判据：这期内容模型再强也吸收不了，因为 ground truth 在外部世界，不在权重里。会过期的只是信号的形式——今天是单测，明天可能是渲染对比、端到端录制。
交付物就是这套 Stop hook 验收配置，在模板仓库第五期目录。作业：把它装进你自己的仓库，跑作业里附带的判定脚本，两个返回码都对才算完成。
...
【画面】
字幕卡：判据"ground truth 在外部世界"→ 模板仓库目录树（高亮 `ep05/`）→ 作业三步骤卡 → Cursor/Codex 对应位置双栏卡
...
1. 准备一个 Node 示例仓库（模板仓库 `ep05/demo/`）：含 `npm run typecheck`（tsc）和 `npm test`（vitest/jest），全套通过耗时 ≤ 20 秒（热路径要求）。
2. 在 `src/calc.ts` 埋一个会导致 `tests/calc.test.ts` 失败的实现（如边界条件错误）。
3. 安装交付物中的 hook 配置（见下节），`chmod +x .claude/hooks/verify-done.sh`。
4. 删除计数器文件：`rm -f "${TMPDIR:-/tmp}/claude-stop-hook-count"`。

### 拍摄流程

1. **配置特写**（30s）：`code .claude/settings.json` → `code .claude/hooks/verify-done.sh`，旁白按逐字稿 0:55–2:35 段落走。
2. **拦截实录**（60–90s）：
   - 启动 `claude`，发任务："在 `src/` 里新增一个 `formatPrice` 函数并导出，做完就结束。"
   - 预期：agent 完成编码后宣布完成 → Stop hook 触发 → `npm test` 失败 → 会话中出现 stderr 塞回的失败摘要 → agent 转去修 `calc.ts` 的失败测试 → 再次宣布完成 → 测试通过 → 放行。
   - **确定性论证**：无论 agent 是否主动跑测试，只要测试处于失败状态，Stop 必被拦（exit 2 是进程级语义，不经过模型判断）。任务本身足够小，agent 一定会尝试结束，不存在"等不到它收尾"的风险。
3. **兜底演示（可选补拍，10s）**：手动往计数器文件写 `3`，再触发一次 Stop，验证直接放行。
...
> 配套（一句话，不重复教学）：测试目录保护复用第 3 期 PreToolUse 保护 hook，把路径从 `generated/` 换成 `tests/` 即可——防止它用"删测试"让验证通过。配置成品在模板仓库 `ep03/`。
...
【画面】
三栏对照卡：Claude Code `settings.json → Stop` | Cursor `.cursor/hooks.json → stop（loop_limit 默认 5）` | Codex `config.toml → [features] hooks`
