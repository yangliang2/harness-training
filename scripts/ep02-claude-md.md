...
> 格式说明：本期 Demo 1（懒加载翻车三拍）与 Demo 2（探针日志对比）为实拍/确定性证据；失效现场用三天三组采样的 Day3 自包含录屏；Demo 3 已撤拍降级为作业素材；作业判据为 check-rules.sh 外部脚本。
...
【画面】
录屏【实拍】：左侧编辑器展示 `frontend/CLAUDE.md` 的位置，右侧终端跑任务。规则"不在场"用协议级证据呈现【实拍】（2026-08-09 实测，布景 `spike/ep02-scope`，InstructionsLoaded 探针日志）：根 CLAUDE.md 的加载记录是 `session_start`，子目录那份是 `nested_traversal`——任务没进那个目录，日志里就永远没有它。翻车后果用实拍录屏【实拍】（`evidence/ep02-lazyload-accident.cast`，第 2 次录制中选），常驻角标【可生成】："实拍实录，选条条件：补丁被覆盖且探针日志零记录"。B 方案见文末。
...
【画面】
左：加载日志卡【可生成，日志条目必须逐字来自探针记录】——根 CLAUDE.md `session_start` vs 子目录 `nested_traversal`，以及"任务没进目录→日志零记录"的对比（2026-08-09 实测，`spike/ep02-scope/`）。右：层级/import/rules paths 语法速查字幕卡【可生成】（观众自读，不占口播；rules 标注"进阶可选"）。录屏【实拍】：把约定从子目录 CLAUDE.md 挪到正确位置（根 CLAUDE.md 或配对 paths 的 rules），跑一次任务，探针日志出现对应条目。字幕【可生成】："本期动作：把'别改 generated/'放到会被加载的位置 + 挂上加载探针"。
...
【画面】
字幕卡【可生成，数字必须逐字来自论文摘要】（静止 4 秒，观众自读）：arXiv 2602.11988 结论三行——总体不提升成功率 / 推理成本平均 +20% / 仅"非标准实践"类指令有正收益。截图【真截】：官方 best practices 的 Exclude 清单三类。角标【可生成】：臃肿样例 `template-repo/ep02/fixtures/bloated-CLAUDE.md`（作业素材）。

### 2:00–3:30 机制二：强制力——建议，不是命令

【口播】
规则瘦身了、作用域也配对了，它能乖乖听话吗？看实测。这是第二个属性：CLAUDE.md 是建议性的，官方原话对照在屏幕上。第一期说过，模型是优化器，优化的是你给的信号而不是你的意图。同一任务，我们三天各跑了 16 次：第一天 15 次违反，第二天 7 次，第三天 7 次——最日常的中性指令，不提文件不提规则，三天各违反 3 次、1 次、2 次。注意两件事：第一，无人施压它自己就会跑偏；第二，违反率每天都在漂移。概率不为零和确定执行之间的差值，就是这么具体。所以问题不是"这次会不会"，是后果你承受得起吗？"别改 generated/"被无视，大不了重新生成；但如果背后是数据迁移、计费逻辑、合规红线呢？先把分工说清楚：这一级管"在不在场"，管不了"听不听话"。承受不起的那条规则，下一期升级成 PreToolUse hook，确定性拦截——同一条规则，一级一级变硬。

【画面】
录屏片段【实拍】（自包含取证版）：中性指令 → agent 直改 generated/ → `git diff` 红框。数据卡【可生成，数据必须逐字来自采样记录】（观众自读）：同一任务 4 种说法 ×16 次 × 三天：15/16、7/16、7/16 违反；时间压力组三天 11/12 最稳定；Day3 逐条录屏 + Day1/2 判定记录 `spike/captures/evidence/ep02/`。字幕卡【可生成，引语必须逐字可核】：官方引语对照——CLAUDE.md 指令是 advisory；hooks 是确定性的，"在固定的生命周期点作为 shell 命令执行，无论模型自己怎么决定都生效"。结尾字幕卡【可生成】："第 3 期：同一条规则 → PreToolUse hook"。
...
【画面】
字幕卡【可生成】：判据三行——"加载机制会变 / 分层生效不变 / 建议与强制的差值永远存在"。
...
【画面】
仓库目录树截图【真截】 + 作业三步字幕卡【可生成】（修剪 bloated-CLAUDE.md / 迁移配 paths / 跑 check-rules.sh 全 PASS）+ Cursor/Codex 对应位置卡【可生成，参数必须逐字可核】（20 秒，见文末）。
...
- 用三天三组采样里 Day3 中性指令的违反录屏（`spike/captures/evidence/ep02/D1-neutral.cast`、`D4-neutral.cast`，自包含：指令横幅→行为→git diff；Day3 为逐条录屏，Day1/2 只有判定记录），数据卡写三天三组（15/16、7/16、7/16；时间压力组 11/12）。
...
`check-rules.sh` 判据（摘要，与 `template-repo/ep02/scripts/check-rules.sh` 一致）：

1. `CLAUDE.md` ≤ 200 行；
2. `CLAUDE.md` 不含目录树 / 依赖列表模式（启发式 grep：`├──`、`npm install`、版本号清单）；
3. 已挂 InstructionsLoaded 加载探针（`log-instructions.sh` 在 settings.json 里）；
4. `.claude/rules/` 为可选项——存在 `.md` 时才校验：每个文件有合法 `paths:` frontmatter 且 glob 非空，且至少一条规则的 paths 与仓库实际目录匹配。
...
【画面】
三栏对照卡【可生成，参数必须逐字可核】：CLAUDE.md + rules paths / `.cursor/rules/*.mdc` 四型触发（Always Apply / 按 globs 自动附加 / 按描述 agent 自取 / 手动 @ 提及；团队规则 1.7 起仪表盘下发）/ AGENTS.md 逐级拼接 + 32KiB 上限。
...
| `/doctor` 自动提议修剪 CLAUDE.md（v2.1.206） | 官方 CHANGELOG + 发布报道 | ✅ 特性存在性已核实；**决策：不拍**（2026-08-07）——版本门槛 + 提议内容模型生成 + 教学核心不依赖它。替代方案 Demo 3 手工修剪后于 2026-08-09 评审撤拍，降级为作业素材（见 Demo 3 节） |
...
| 失效现场录屏：Day3 自包含录屏逐条在册（A1–D4 + verdicts-v2.csv），中性指令违反条 D1/D4 | `spike/captures/evidence/ep02/`（MANIFEST ep01 节三天三组表） | ✅ 已核实（2026-08-16 复审）：文件存在，判定与口播"3 次、1 次、2 次"及合计 15/16、7/16、7/16、时间压力组 11/12 一致；Day1/2 仅判定记录，字幕已如实标注覆盖范围 |
| 翻车导入录屏：`evidence/ep02-lazyload-accident.cast`（第 2 次录制中选，补丁 TIMEOUT_MS 3000→1000 被冲回 + 探针日志零记录） | MANIFEST ep02 节 + cast 原文 | ✅ 已核实（2026-08-16 复审）：cast 存在且内容支持声称；脚本内旧表述"策展 transcript／选自 N 次运行中的典型样本"已改为"实拍实录，选条条件：补丁被覆盖且探针日志零记录" |
| 三天采样口径用"同一任务"而非"同一布景"（Day1 规则载体为 rules 文件、Day3 起为根 CLAUDE.md，机制等价） | MANIFEST ep01 节拍摄口径注 | ✅ 已遵守（2026-08-16 复审修复：口播"同一个布景"残留已改"同一任务"） |
| check-rules.sh 四条判据摘要（含探针挂载判据、rules 为可选项） | `template-repo/ep02/scripts/check-rules.sh` 原文 | ✅ 已核实（2026-08-16 复审）：`bash -n` 通过，判据为外部脚本不靠自我报告；脚本内摘要旧版（漏探针判据、rules 误标为必校验）已对齐实际脚本 |
| 内嵌交付物 `CLAUDE.md` 与 `.claude/rules/generated.md` 和模板仓库逐字一致 | diff `template-repo/ep02/` | ✅ 已核实（2026-08-16 复审，diff 无差异） |
