...
【画面】
录屏片段（自包含取证版）：中性指令 → agent 直改 generated/ → `git diff` 红框。数据卡（观众自读）：同一布景 4 种说法 ×16 次 × 三天：15/16、7/16、7/16 违反；时间压力组三天 11/12 最稳定；逐条录屏 `spike/captures/evidence/ep02/`。字幕卡：官方引语对照——CLAUDE.md 指令是 advisory；hooks 是确定性的，"在固定的生命周期点作为 shell 命令执行，无论模型自己怎么决定都生效"。结尾字幕卡："第 3 期：同一条规则 → PreToolUse hook"。
