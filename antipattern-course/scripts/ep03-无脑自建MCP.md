...
> 证据提示：`spike/` 不入库；**开拍前待办两项**：a) KB spike 三臂 A/M0/M1（千篇级夹具、任务混合预登记、翻车阈值预登记，见 Demo 步骤）；b) **企业出镜面清查**（企业内部自建 MCP 清单、description 现状留档、可拍边界与公司确认——见布景分工）。
...
【画面】
承接卡 1 秒【可生成】→ **企业内部真实自建 MCP 工具列表滚动**【实拍，出镜面清查后】（真实工具名一排即"工具=MCP 条件反射"的第一人称证据；不可拍则回退夹具 M0 工具列表）。**防混淆**：不出现输出刷屏（第 2 集的病）、不出现 `/context` 工具定义占用（第 1 集的病）——本集企业镜头只拍工具面与说明面。
...
【画面】
"市面普通水平"落句处插**企业真实 server 的 description 特写一闪**【实拍，出镜面清查后】（空白/一句话敷衍的真实说明，字幕："这不是我们编的"；不可拍则用公开 server 真实烂 description 真截替代【真截】）→ 三栏分屏实拍【实拍，KB spike 夹具】→ 结果数字卡【可生成，占位待填，字幕标 N 与任务混合比例】。
...
### 布景分工（2026-08-24 决议，先例同 ep02）

| 段落 | 布景 | 性质 |
|---|---|---|
| 开场工具列表滚动 + M1 段真实 description 特写 | **企业内部真实自建 MCP** | 第一人称犯病率证据（工具=MCP 反射存在 + 说明烂是真实现状） |
| 三臂对照、逐件对照演示、全部数字镜头 | **KB 夹具**（可控、可复现、可选条） | 数字纪律要求 |

**企业实拍红线（拍前逐项过）**：知识库正文/文档标题/作者名一律不出镜——只拍工具列表与 description 字段；内部系统名/人名/项目代号/内网 URL/凭据字段不出镜；description 含业务信息时用局部特写+遮挡，或按"换皮重跑纪律"（同 ep02）搭测试 server 复刻同等说明质量再拍——**真实环境只证明存在性，数字一律出自夹具**；拍前与公司确认边界。

**回退方案（企业无自建 KB MCP 或不可拍）**：开场工具列表回退夹具 M0；description 特写改公开 server 真实烂说明真截（arXiv:2602.14878 数据集内样本或 GitHub 公开 server，逐字真实不编）。
